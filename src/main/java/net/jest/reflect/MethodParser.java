...
public class MethodParser {
    ...
    public String getName(@NonNull Controller controller, @NonNull java.lang.reflect.Method method) {
        ...
        print("Found method with path = " + path);
        return path;
    }
    ...
    public Function<Request, Response> parse(@NonNull Controller controller, @NonNull Object instance, @NonNull java.lang.reflect.Method method) {
        ...
        Map<Class<?>, String> parameters = new HashMap<>();
        ...
        return request -> {
            ...
            parameters.forEach((type, name) -> {
                try {
                    if (request.getParameter(type, name) == null)
                        response.set(ResponseUtil.createResponse(ResponseUtil.BAD_REQUEST,
                                "The " + name + " argument was not found, or the wrong type was specified. (The message is generated by the Jest framework)"));
                    else parametersInvoke.add(request.getParameter(type, name));
                } catch (Exception e) {
                    response.set(ResponseUtil.createResponse(ResponseUtil.INTERNAL_SERVER_ERROR,
                            "Internal server error during process request. (The message is generated by the Jest framework)"));
                }
            });
            ...
            try {
                if (method.getReturnType().isAssignableFrom(Response.class) || method.getReturnType().equals(Response.class)) return (Response) method.invoke(instance, parametersInvoke.toArray(new Object[1]));
